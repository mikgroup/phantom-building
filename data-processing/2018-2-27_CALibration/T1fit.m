...
t1show = imresize(squeeze(mean(mean(T1est,3),4)),'scale',[1 2]);
mask_show = imresize(squeeze(mean(mean(mask,3),4)), 'scale', [1 2]);
st(1000*bsxfun(@times, t1show(130:380,:), mask_show(130:380,:)), [0, 1500]); colormap('parula'), colorbar;
title('T1 map (ms)', 'FontSize', 30); faxis
%%
