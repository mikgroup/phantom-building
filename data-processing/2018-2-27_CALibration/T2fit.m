...
%%
t2show = imresize(squeeze(mean(mean(T2est,3),4)),'scale',[1 2]);
mask_show = imresize(squeeze(mean(mean(T2mask,3),4)), 'scale', [1 2]);
st(1000*bsxfun(@times, t2show(130:380,:), mask_show(130:380,:)), [0, 170]); colormap('parula'), colorbar;
title('T2 map (ms)', 'FontSize', 300); faxis
