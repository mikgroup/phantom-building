...
plot(t2_vals([1:size(t1_C,2)]), t1_vals([1:size(t1_C,2)]), '+-', 'linewidth', 3, 'MarkerSize', 10)
plot(t2_vals([12:16]), t1_vals([12:16]), '+-', 'linewidth', 3, 'MarkerSize', 10, 'Color', [0.4940, 0.1840, 0.5560])
plot(t2_vals([8,7,6,10,11]), t1_vals([8,7,6,10,11]), '+-', 'linewidth', 3, 'MarkerSize', 10, 'Color', [0.4940, 0.1840, 0.5560])
plot(t2_vals([7,9,10]), t1_vals([7,9,10]), '+-', 'linewidth', 3, 'MarkerSize', 10, 'Color', [0.4940, 0.1840, 0.5560])
plot(t2_vals(1:2:end), t1_vals(1:2:end), 'ro', 'linewidth', 3, 'MarkerSize', 10, 'Color', [0.4940, 0.1840, 0.5560])
hold off
grid on
grid minor
xlabel('T_2 (ms)');
...
xlim([10, 250]);
ylim([10, 2500]);
legend({sprintf('%s bound', ions{ion1}), sprintf('%s bound', ions{ion2}), 'Target', 'Experimental'}, 'Location', 'northwest');
faxis(gca, 20);
...
t1err = abs((t1_vals - t1mix')./t1mix' * 100);
t2err = abs((t2_vals - t2mix')./t2mix' * 100);
[~,i] = sort(t1mix);
fprintf([repmat('%0.2f &\t', 1, 6) '%d \\\\ \n'] ,[t1mix(i)' , t1_vals(i) , t1err(i), t2mix(i)', t2_vals(i), t2err(i),i' ]')
