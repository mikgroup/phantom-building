...
rW1fixed = mean(rW1) + 16.0680 * sol(3);
rW2fixed = mean(rW2) + 16.0680 * sol(6);

sol = [0.516830015067671;7.408756428145526;0.607309505279191;1.043974238027491e+02;0.324249968831636;5.045795853972622];
%sol = [0.6179064;8.692354;1.015674;112.0304;0.3871927;5.007029];
m1 = [sol(1), sol(2)];
rW1 = [sol(5), sol(5)];
m2 = [sol(3), sol(4)];
rW2 = [sol(6), sol(6)];
...
r2fun_gen = @(ii, r2) m1(ii) / m2(ii) * (r2 - rW2fixed) + rW1fixed;
t2fun_gen = @(ii, r2) m2(ii) / (m1(ii)*(r2 - rW2fixed) + rW1fixed);
...
r2max = 30;
r1max = 7;
% [x, y] = meshgrid(linspace(0,35,10), linspace(0,10,10));
% x = [x(:), y(:)];
% x = x(x(:,1) < r2max, :);
% x = x(x(:,2) < r1max, :);
% x = x(x(:,2) < r2fun_gen(ion1, x(:,1)), :);
% x = x(x(:,2) > r2fun_gen(ion2, x(:,1)), :);
% 
% r2mix = x(:,1);
% r1mix = x(:,2);
...
t2min = 20;
t1min = 190;
[x, y] = meshgrid(linspace(t2min,250,10), linspace(t1min,2000,10));
x = [x(:), y(:)];
x = x(x(:,1) > t2min, :);
x = x(x(:,2) > t1min, :);
x = x(1000./x(:,2) < 0.95 * r2fun_gen(ion1, 1000./x(:,1)), :);
x = x(1000./x(:,2) > 1.05 * r2fun_gen(ion2, 1000./x(:,1)), :);


t1mix = [391.11111,592.22222,391.11111,592.22222,793.33333,391.11111,592.22222,793.33333,994.44444,1195.5556,592.22222,793.33333,994.44444,1195.5556,1396.6667,793.33333,994.44444,1195.5556,1396.6667,1597.7778,994.44444,1195.5556,1396.6667,1597.7778,1798.8889,1597.7778,1798.8889,2000];
t2mix = [45.55556,45.55556,71.11111,71.11111,71.11111,96.66667,96.66667,96.66667,96.66667,96.66667,122.2222,122.2222,122.2222,122.2222,122.2222,147.7778,147.7778,147.7778,147.7778,147.7778,173.3333,173.3333,173.3333,173.3333,173.3333,198.8889,198.8889,198.8889];
...
hold on
%plot(1000./t2_vals, 1000./t1_vals, '+', 'linewidth', 3, 'MarkerSize', 10)
plot(r2_range, r1max * ones(size(r2_range)), 'linewidth', 3);
plot(r2max * ones(size(r2_range)), r2_range, 'linewidth', 3);
plot(r2mix, r1mix, '+', 'linewidth', 3, 'MarkerSize', 10)
hold off
grid on
grid minor
xlabel('R_2 (1/s)');
